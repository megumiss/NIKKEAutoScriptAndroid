...
设置子页：STAR 验证、初始化 NKAS、后端地址、原始 WebUI、更新、控制连接、部署。子页显示返回按钮并隐藏底部导航。页面标题只在 AppBar 出现一次；正文可使用“运行概览”“实例状态”“日志文件”等模块标题，但不重复页面标题。
...
固定尺寸：图标按钮视觉 40x40（触控至少 44x44），主按钮最小高度 38，Sheet 操作至少 44，设置行至少 56，实例行至少 68，任务行至少 55，底部导航 344x58，导航单项至少 46x46。
...
常用语义：总览 `layout-dashboard`、实例 `layers-3`、画面 `monitor-play`、日志 `scroll-text`、设置 `settings-2`、刷新 `refresh-cw`、启动 `play`、停止 `square`、返回 `arrow-left`、更新 `square-arrow-up`、验证 `shield-check/github`、初始化 `sparkles/rocket`、后端 `server`、WebUI `globe-2/external-link`。
...
原型是居中的白色半透明胶囊：宽 344px、高 58px、底部 14px + SafeArea、六项等宽。选中为 `blueSoft` 背景、蓝色图标和底部 4px 小圆点。只切换总览、实例、任务、画面、日志、设置，不放启停或更新；部署页已挪入设置子页（设置 → 部署），对应后端 deploy 配置。

### Tabs

「实例」详情页使用横向 tab 条：运行中、队列中、等待中、实时日志在页面上方横向排列；「任务」页使用同样的横向 tab 条：任务配置、调度设置。tab 超出可横向滚动；选中胶囊为 `blueSoft` 底 + 蓝色字，未选中为 muted。「画面」是一级页面，不在实例页或任务页内。
...
一级页面，直接进入选中实例的详情（不再有列表层）：顶部紧凑头部（头像 + 实例名 + 状态 | 切换 | 启动/停止），其下横向 tab 条：运行中、队列中、等待中、实时日志。队列三段各为完整列表并带计数，点行跳任务页对应配置；实时日志为实例运行输出的实时流。多实例通过头部「切换」底部 Sheet 更换。

### 任务

一级页面。顶部精简实例切换条（同画面页规格），其下横向 tab 条：任务配置、调度设置。任务配置按 schema 的菜单/任务/分组动态生成，调度按真实 cadence 和锁定状态生成。实时日志协议未确定前不得显示假数据或“实时 2s”。

### 画面

一级页面。顶部是精简实例切换条：头像 + 实例名 + 状态 | 切换（弹底部 Sheet 选实例），不放启停按钮。下方是 `GET /api/{name}/screenshot` 的 2 秒轮询预览（深色表面、顶部状态条），未取到帧显示“未连接 / 暂无画面”空态。打开页面只轮询截图，不自动开启控制；点状态条「连接设备」才启动原生 scrcpy 实时画面与触控，断开后恢复截图轮询。未通过 STAR 验证时不启动轮询。
...
设置使用分组行，不重复显示“设置”正文标题。顺序为验证与初始化、后端连接、外观与通知、关于；更新放在后端连接分组最下面。「关于」是底部一行入口，进入独立子页展示应用版本、项目链接（仓库、问题反馈）和运行信息（后端版本、后端地址、API 版本、技术栈）。主题和语言是 App 自己的本地设置，不调用 WebUI 主题/语言接口。Android 的 STAR 验证、初始化和本地服务启停通过平台桥复用既有原生流程；iOS 隐藏 Termux/ADB 内容。
...
- [ ] 390px、360px 手机无横向溢出；
- [ ] 六项底部导航位置、尺寸、选中态与原型一致；
- [ ] 总览只有一个浅蓝 Hero，无渐变；
- [ ] 实例头部一行显示状态、切换、启停；
- [ ] 实例页多实例先进列表层、单实例直达详情层，功能层可返回；
