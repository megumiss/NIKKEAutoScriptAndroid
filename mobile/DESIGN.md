...
实例页不再使用 Tab。实例页内部是自适应两级结构：多实例时先显示实例列表，点卡片进入该实例详情；任务配置、调度设置、实时日志是详情之下的整页功能层，顶部为返回条（返回箭头 + 标题）。单实例时跳过列表层，一级页面直接是实例详情。「画面」是一级页面，不在实例页内。
...
自适应两级结构，层级在实例页内部管理（壳层持有 `InstanceLayer` 状态）：

- **列表层**（仅实例数 > 1 时出现）：实例卡片列表，每张卡含头像、名称、状态点、当前/下一任务摘要、队列计数（运行/队列/等待 n/n/n）、启停按钮和 chevron；点卡片进入该实例详情层，启停按钮按卡片实例操作。
- **详情层**（单实例时一级页面直接是这一层）：顶部紧凑头部（头像 + 实例名 + 状态 | 切换（多实例时显示）| 启动/停止）；队列摘要 Surface（运行中/队列中/等待中计数 + 队列行，点行进任务配置层）；画面缩略 Surface（进入时拉一帧截图，不轮询，点击跳画面页）；「管理」入口列表（任务配置 / 调度设置 / 实时日志）。
- **功能层**：任务配置 / 调度设置 / 实时日志整页展示，顶部返回条返回详情层。任务配置按 schema 的菜单/任务/分组动态生成，调度按真实 cadence 和锁定状态生成。实时日志协议未确定前不得显示假数据或“实时 2s”。
