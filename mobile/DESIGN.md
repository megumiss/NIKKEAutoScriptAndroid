...
实例页不使用 Tab，一级页面直接是选中实例的详情。「任务」页使用横向 tab 条：任务配置、运行中、队列中、等待中、实时日志、调度设置在页面上方横向排列，超出可横向滚动；选中胶囊为 `blueSoft` 底 + 蓝色字，未选中为 muted。「画面」是一级页面，不在实例页或任务页内。
...
一级页面，直接进入选中实例的详情（不再有列表层）：顶部紧凑头部（头像 + 实例名 + 状态 | 切换 | 启动/停止），下方队列摘要 Surface（运行中/队列中/等待中计数 + 队列行，点行跳任务页对应配置）。多实例通过头部「切换」底部 Sheet 更换。

### 任务

一级页面。顶部精简实例切换条（同画面页规格），其下横向 tab 条：任务配置、运行中、队列中、等待中、实时日志、调度设置，超出可横向滚动。队列三段各为完整列表，点行进任务配置 tab 对应任务。任务配置按 schema 的菜单/任务/分组动态生成，调度按真实 cadence 和锁定状态生成。实时日志协议未确定前不得显示假数据或“实时 2s”。
