...
原型是居中的白色半透明胶囊：宽 344px、高 58px、底部 14px + SafeArea、六项等宽。选中为 `blueSoft` 背景、蓝色图标和底部 4px 小圆点。只切换总览、实例、画面、日志、部署、设置，不放启停或更新；部署页对应后端 deploy 配置。

### Tabs

实例 Tab 使用横向滚动文字 Tab：`概览 | 任务配置 | 调度设置 | 实时日志`。激活态为蓝色文字和底部 2px 指示线；必须单行，超出屏幕横向滚动。「画面」已拆出为一级页面，不再出现在实例 Tab 中。
...
顶部是一行紧凑头部：头像 + 实例名 + 状态 | 切换 | 启动/停止。不要再放重复实例列表卡片。概览只显示后端 `running/pending/waiting` 分组，不增加无接口依据的统计卡片。任务配置按 schema 的菜单/任务/分组动态生成，调度按真实 cadence 和锁定状态生成。实时日志区域保留原型位置，但协议未确定前不得显示假数据或“实时 2s”。

### 画面

一级页面。顶部是精简实例切换条：头像 + 实例名 + 状态 | 切换（弹底部 Sheet 选实例），不放启停按钮。下方是 `GET /api/{name}/screenshot` 的 2 秒轮询预览（9/16 比例、深色表面、顶部状态条），未取到帧显示“未连接 / 暂无画面”空态；取到帧后状态条按钮变为「进入控制」，打开原始 WebUI 控制入口。未通过 STAR 验证时不启动轮询。
