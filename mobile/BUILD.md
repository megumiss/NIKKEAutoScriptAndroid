...
本文面向 `mobile/` 下的 Flutter 客户端，命令使用 Windows PowerShell。Windows 可以生成 Android APK；iOS 本地构建需要 macOS 和 Xcode，步骤见 [移动端说明](README.md#ios-构建)。

## 这台电脑直接打包

当前工程在 `D:\PCR\NIKKEAutoScriptAndroid\mobile`，Flutter、Android SDK、原生 AAR 和正式版签名均已准备好。在 PowerShell 中执行：

```powershell
Set-Location 'D:\PCR\NIKKEAutoScriptAndroid\mobile'
...
```text
D:\PCR\NIKKEAutoScriptAndroid\mobile\build\app\outputs\flutter-apk\app-release.apk
```

把 APK 复制到手机后打开安装即可。每次构建会覆盖这个文件，版本名称取自 `mobile/pubspec.yaml`。
...
脚本会下载固定版本的 gomobile、gobind 和 Go 依赖，生成 `mobile/native/android/nkas-tsnet.aar`，包含 `armeabi-v7a`、`arm64-v8a`、`x86_64` 三种架构。该文件不提交到 Git，因此新克隆的工程需要先生成它。

AAR 缺失，或 `mobile/native/tsnet/` 中的 Go 源码、依赖、绑定构建参数发生变化时，重新运行此脚本。首次 Gradle 构建还会自动下载并校验 scrcpy server。
...
以下命令在 `mobile/` 中执行，并使用上面配置的 Flutter PATH。Debug 构建同样需要先生成 tsnet AAR，但使用 Android 调试签名：
...
产物是 `mobile/build/app/outputs/flutter-apk/app-debug.apk`。日常安装使用 Release 包；Debug 与 Release 的签名通常不同，不能直接互相覆盖。
...
| `Missing native tsnet binding` | 在 `mobile/` 执行 `python -X utf8 tool/build_tsnet.py android`，确认生成 `native/android/nkas-tsnet.aar`。 |
| 找不到 SDK、NDK 或未接受许可 | 检查 `ANDROID_HOME`、NDK 28.2.13676358 和 `flutter doctor --android-licenses`。 |
| `local.properties` 缺失或 `flutter.sdk not set` | 先从 `mobile/` 执行 `flutter build apk --debug --config-only`，再直接调用 Gradle。 |
...
每次 Git 提交前，在 `mobile/` 中递增版本号，并同步关于页展示：
...
在 `mobile/` 中使用 PowerShell 生成 Base64，密钥文件名按实际配置替换：
