...
| Flutter analyze / test | analyze 无问题；47 项测试通过，包含控制连接页面的保存、返回、取消注册和画面重连 |
...
| 场景 | 操作 | 预期 |
| --- | --- | --- |
| 控制连接页面 | 从设置或画面页进入“控制连接”，编辑后分别保存和返回；小屏下弹出键盘 | 显示完整页面及顶部返回按钮；保存后返回，未保存的编辑不提交；输入项和按钮可滚动访问，画面入口返回后重新连接 |
