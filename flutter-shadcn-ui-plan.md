...
> 本文保留早期视觉预览与架构计划，当时使用假数据；当前实现与构建入口见 [README.md](README.md)。Flutter 工程已上移到仓库根目录。
> 视觉规范以 `DESIGN.md` 为唯一依据，组件库使用 `shadcn_ui`。
...
```text
本仓库根目录                 Flutter：Android/iOS 统一控制界面
android/                     Flutter Android 宿主：初始化、Termux、原生控制与后台服务
分支 codex/legacy-kotlin      旧版独立 Kotlin Android 客户端
NIKKEAutoScript/webui/        Vue WebUI：现有完整功能与配色参考
NIKKEAutoScript/module/webui/ Starlette REST/WebSocket 服务
```
...
接口映射以 `doc/flutter-backend-mapping.md` 为准。现有 Starlette API 的关键端点是：
