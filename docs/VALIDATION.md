...
- 初始化步骤日志首次显示、重新展开和追加输出时默认跟随末尾。手动上翻暂停跟随，回到底部后恢复；出现错误时滚到错误信息处，后续输出保留当前阅读位置。单行日志仍按内容收缩，长日志最高 190px。
- Windows / Flutter 3.47.2：修改文件定向分析通过，`flutter test --no-pub test/setup_page_test.dart` 共 15 项通过。修复前已复现长日志滚动位置始终为 0；新增覆盖短日志增长、自动跟随、重新展开、手动阅读、恢复跟随和错误可见性。本次未打包、安装或运行真机验证。
