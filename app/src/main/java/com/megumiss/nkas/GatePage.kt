...
/** 项目授权页：由 GateActivity 迁移为单 Activity 内的页面渲染器。 */
class GatePage(private val activity: Activity, private val navigate: (String) -> Unit) {
    ...
    private fun renderAuthorized(license: AccessGate.License) {
        ...
        panel.addView(TextView(activity).apply { text = "项目授权已完成"; textSize = 24f; setTextColor(Ui.text); setTypeface(Typeface.DEFAULT, Typeface.BOLD); gravity = Gravity.CENTER; setPadding(0, dp(16), 0, dp(8)) })
        panel.addView(TextView(activity).apply { text = "GitHub 账号：${license.username}\n授权有效期至：${formatDate(license.expiresAt)}"; textSize = 14f; setTextColor(Ui.text2); gravity = Gravity.CENTER; setPadding(0, dp(8), 0, dp(20)) })
        ...
        val refresh = Button(activity).apply { text = "重新检查授权"; textSize = 13f; Ui.styleSecondary(activity, this); setOnClickListener { beginAuthorization() } }
        panel.addView(refresh, LinearLayout.LayoutParams(-1, dp(48)).apply { topMargin = dp(10) })
        content.addView(panel, LinearLayout.LayoutParams(-1, -2))
    }

    /** 处理 nkas://auth 授权回调；非回调 Intent 时按本地授权状态渲染。 */
